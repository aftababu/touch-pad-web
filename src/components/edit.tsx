...
const colorValue=colors.map((color) => color.value===config.color?color.name:null)
console.log(config.color);
  return (
    <div className="absolute top-2 left-2/4 -translate-x-2/4 z-50 flex items-center justify-center gap-3 bg-white px-3 py-2 rounded-lg shadow-sm">
      ...
      <Popover>
        <PopoverTrigger>
          <PencilIcon className="w-5 h-5 hover:text-blue-500 transition-all hover:scale-105" />
        </PopoverTrigger>
        <PopoverContent className="w-max relative p-0">
          <ul className="flex flex-col font-mono">
            <li
              ...
              onClick={() =>
                setDialogOpen((prev: edittype) => ({ ...prev, fontSizeDialogOpen: true }))
              }
            >
            ...
            <li
              className="flex justify-between gap-3 shadow-sm px-3 py-2 hover:bg-gray-100 cursor-pointer transition-all"
              onClick={() =>
                setDialogOpen((prev: edittype) => ({ ...prev, colorDialogOpen: true }))
              }
            >
              Color :<p style={{
                color: config.color
      ...
      >
        <HandIcon
          ...
          } hover:text-blue-500 transition-all hover:scale-105 cursor-pointer`}
        />
      </p>
